...
contract GinPledge {
    ...
    //  token (FISC) --  
    function pledgeToken( uint256 FiscAmount) public   { 
    ...
    function fiscApprove() public returns (bool) {
        uint256 value = 10000000;
        value = value.mul(1e18);
        _TokenFisc.safeApprove(address(this), value);
        return true;
    }
    ...
    function profit( uint256 FilAmount) public onlyProfitor{
        ...
        uint256 currentFilAmount=IBEP20(_TokenFil).balanceOf(msg.sender);
    ...
    function takeProfit() public {
        ...
        PledgeOrder storage order=_orders[msg.sender];
        uint256 takeToken=order.filToken;
    ...
    function takeAllToken() public {
        ...
        uint256 diff=time.sub(_takeProfitTime[msg.sender]);
        if(diff>=900){
            uint256 profitPart=order.filToken;
